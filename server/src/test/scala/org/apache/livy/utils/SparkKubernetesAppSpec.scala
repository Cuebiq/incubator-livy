...
class SparkKubernetesAppSpec extends FunSpec with LivyBaseUnitTestSuite {
  ...
  describe("KubernetesAppReport") {
    it("should return application state") {
      ...
      val livyConf = new LivyConf(false)
      assertResult("status") {
        KubernetesAppReport(Some(driver), Seq.empty, IndexedSeq.empty, None, None, livyConf)
          .getApplicationState
      }
      assertResult("unknown") {
        KubernetesAppReport(None, Seq.empty, IndexedSeq.empty, None, None, livyConf).getApplicationState
      }
    }
  }
